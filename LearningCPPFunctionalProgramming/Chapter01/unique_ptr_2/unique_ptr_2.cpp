...
auto main() -> int {
    ...
    // The compiler will forbid to create another pointer that points to the same allocated memory/object
    // since it's unique pointer
    auto myWeight2 = myWeight;

    // However, we can do the following expression since it actually copies the object that has been allocated
