...
int main() {
    ...
    std::ranges::copy(
        lessThan12 | std::views::reverse
                   | std::views::transform([](int x) {return x * x;}), 
                   output);
    
    std::cout << "\nSkip 1000 even ints, then take five: ";
    std::ranges::copy(
        evens | std::views::drop(1000) 
              | std::views::take(5),
              output);
    
    std::cout << "\nFirst five even ints greater than 1000: ";
    std::ranges::copy(
        evens | std::views::drop_while([](int x) {return x <= 1000;})
              | std::views::take(5),
              output);
    ...
    std::ranges::copy(romanNumerals | std::views::elements<0>, stringOutput);
