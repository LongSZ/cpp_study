// Fig. 17.18: SemaphoreLatch.cpp
// Coordinate threads with a custom latch implemented 
// with a semaphore and an atomic_int.
#include <chrono>
...
int main() {
    ...
    // latch that three threads must signal before main continues
    MyLatch mainLatch{3};

    // lambda representing the task to execute
    auto task{
        [&](std::string name, std::chrono::milliseconds workTime) {
    ...
    for (int i{1}; i < 4; ++i) {
        // create jthread that calls task lambda, passing a task name and work time
        threads.emplace_back(std::jthread{task, fmt::format("Task {}", i), std::chrono::milliseconds{ints(random)}});
    }
