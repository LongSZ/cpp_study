...
int main() {
    ...
    auto paintingRobotTask{
        [&](std::string name) {
            // check whether the assembly line is shutting down and, if not, do the painting work
            while (!stopToken.stop_requested()) {
                ...
                // synchronize access to the random-number generator
                {
                    ...
                    workTime = std::chrono::milliseconds{ints(random)};
                }
                ...
                std::cout << fmt::format("{} done painting. Waiting for next car.\n", name);

                // decrement paintingDone barrier's counter and wait for other painting robots to arrive here
    ...
    std::jthread righttSideRobot{paintingRobotTask, "Right side robot"};
}
