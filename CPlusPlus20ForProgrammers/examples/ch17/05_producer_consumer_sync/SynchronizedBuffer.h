...
private:
   ...
   mutable bool m_occupied{false}; // mutable so get() can modify
   mutable std::condition_variable m_cv; // mutable so get() can modify
   mutable std::mutex m_mutex; // mutable so get() can modify
};
