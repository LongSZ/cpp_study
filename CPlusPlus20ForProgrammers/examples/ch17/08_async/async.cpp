...
// type alias for a tuple containing a task name, a number, whether the number prime and its factors 
...
FactorResults getFactors(std::string name, long long number) {
    std::cout << fmt::format("{}: Thread {} executing getFactors for {}\n", name, id(), number);
...
void proveFactors(long long number, Factors factors) {
    ...
    // for each factor/count pair, unpack it then multiply proof by factor the number of times specified by count
...
void displayResults(FactorResults results) {
    // unpack results into name (std::string), number (long long), isPrime (bool) and factors (vector) 
...
int main() {
    ...
    auto future1{std::async(std::launch::async, getFactors, "Task 1", 1016669006116682993)};    // not prime
    auto future2{std::async(std::launch::async, getFactors, "Task 2", 1000000000000000003)};    // prime
