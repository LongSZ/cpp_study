...
// calculate the distance (number of items) between two iterators; requires at least input iterators
template <std::input_iterator Iterator>
auto customDistance(Iterator begin, Iterator end) {
    // for random-access iterators subtract the iterators 
    if constexpr (std::is_base_of_v<std::random_access_iterator_tag, Iterator::iterator_category>) {
        ...
        std::cout << "Called customDistance with non-random-access iterators\n";
        std::ptrdiff_t count{0};
        // increment from begin to end and count number of iterations; O(n) operation for non-random-access iterators
